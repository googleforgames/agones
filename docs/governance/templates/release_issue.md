...
- [ ] Update the `agones` crate version in `sdks/rust/Cargo.toml` to {version}. Commit the changes in a feature branch called `prep/{version}+1`. Run `make sdk-publish-rust`. This command executes `cargo login` for authentication, performs a dry-run publish, and if that succeeds, does the actual publish. Will need [crate's API TOKEN](https://crates.io/settings/tokens) from your crate's account.
