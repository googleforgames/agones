...
locals {
  ...
  kubernetesVersion       = lookup(var.cluster, "kubernetesVersion", "1.22")
...
# purely optional therefore commeted out
# resource "null_resource" "test-setting-variables" {
#   provisioner "local-exec" {
#     command = <<EOT
#     ${format("echo Current variables set as following - name: %s, project: %s, machineType: %s, initialNodeCount: %s, network: %s, zone: %s, windowsInitialNodeCount: %s, windowsMachineType: %s, minNodeCount: %s, maxNodeCount: %s",
#     local.name,
#     local.project,
#     local.machineType,
...
resource "google_container_cluster" "primary" {
  ...
  # since we need workload identity as per https://github.com/googleforgames/agones/tree/main/examples/allocation-endpoint
  workload_identity_config {
    workload_pool = "${data.google_project.project.project_id}.svc.id.goog"
  }
}

# we are assuming that client will connect to server using UDP from anywhere in the world so... it needs to be allowed to happen :) 
