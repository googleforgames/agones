...
For example, if you have an in-game event happening between 12:00 AM and 10:00 PM on October 31, 2024 EST, and you want to ensure there are always at least 5 ready game servers during that time, you chain add a schedule policy that applies a buffer policy with a buffer size of 5. Now, lets say you want to adjust the number of game servers after the event based on external factors after the in-game event you could chain a webhook policy to dynamically. Finally as a fallback, lets say webhook invocation fails you can chain another buffer policy to default to.
