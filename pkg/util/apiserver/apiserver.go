...
// APIServer provide the routes for managing Kubernetes API extensions
...
func NewAPIServer(mux *http.ServeMux) *APIServer {
	...
	// we don't *have* to have a swagger api, so just do an empty one for now
	// kube-openapi looks awesome for the future.
...
// AddAPIResource stores the APIResource, and returns it
// in the appropriate place for the K8s discovery service
// e.g. http://localhost:8001/apis/scheduling.k8s.io/v1beta1
func (as *APIServer) AddAPIResource(gv string, resource metav1.APIResource, handler CRDHandler) {
	list, ok := as.resourceList[gv]
	if !ok {
		// discovery handler
		list = &metav1.APIResourceList{GroupVersion: gv, APIResources: []metav1.APIResource{}}
		as.resourceList[gv] = list
		pattern := fmt.Sprintf("/apis/%s", gv)
		as.addSerializedHandler(pattern, list)
		as.logger.WithField("groupversion", gv).WithField("pattern", pattern).Info("Adding Discovery Handler")

		// e.g.  /apis/stable.agones.dev/v1alpha1/namespaces/default/gameservers
		// CRD handler
		pattern = fmt.Sprintf("/apis/%s/namespaces/", gv)
		as.mux.HandleFunc(pattern, https.ErrorHTTPHandler(as.logger, as.resourceHandler(gv)))
		as.logger.WithField("groupversion", gv).WithField("pattern", pattern).Info("Adding Resource Handler")
	}

	// discovery resource
	list.APIResources = append(as.resourceList[gv].APIResources, resource)

	// add specific crd resource handler
	key := fmt.Sprintf("%s/%s", gv, resource.Name)
	as.delegates[key] = handler

	as.logger.WithField("groupversion", gv).WithField("apiresource", resource).Info("Adding APIResource")
}

// resourceHandler handles namespaced resource calls, and sends them to the appropriate delegate
