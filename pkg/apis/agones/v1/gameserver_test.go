...
func TestGameServerPodStaticTCPUDP(t *testing.T) {
	t.Parallel()

	// Test case of static for TCPUDP Protocol
	fixture := defaultGameServer()
	fixture.ObjectMeta.Name = "test-pod1"
	fixture.Spec.Ports[0].Protocol = ProtocolTCPUDP
	fixture.ApplyDefaults()
	// Call the Pod function
	pod, err := fixture.Pod(fakeAPIHooks{})
	// Check for errors
	require.NoError(t, err)
	assert.Equal(t, "test-pod1", pod.Spec.Hostname)
	// Assert that the resulting pod has the expected ContainerPort entries
	assert.Equal(t, 2, len(pod.Spec.Containers[0].Ports))
	// You can further assert the specific values if needed
	assert.Equal(t, int32(7777), pod.Spec.Containers[0].Ports[0].ContainerPort)
	assert.Equal(t, int32(7777), pod.Spec.Containers[0].Ports[1].ContainerPort)
	assert.Equal(t, corev1.ProtocolTCP, pod.Spec.Containers[0].Ports[0].Protocol)
	assert.Equal(t, corev1.ProtocolUDP, pod.Spec.Containers[0].Ports[1].Protocol)
	assert.Equal(t, int32(9999), pod.Spec.Containers[0].Ports[0].HostPort)
	assert.Equal(t, int32(9999), pod.Spec.Containers[0].Ports[1].HostPort)

	// Test case of static for TCP Protocol
	fixture = defaultGameServer()
	fixture.ObjectMeta.Name = "test-pod2"
	...
	// Check for errors
	require.NoError(t, err)
	assert.Equal(t, "test-pod2", pod.Spec.Hostname)
	// Assert that the resulting pod has the expected ContainerPort entries
	assert.Equal(t, 1, len(pod.Spec.Containers[0].Ports))
	// You can further assert the specific values if needed
	assert.Equal(t, int32(7777), pod.Spec.Containers[0].Ports[0].ContainerPort)
	assert.Equal(t, corev1.ProtocolTCP, pod.Spec.Containers[0].Ports[0].Protocol)
	assert.Equal(t, int32(9999), pod.Spec.Containers[0].Ports[0].HostPort)

	// Test case of static for UDP Protocol
	fixture = defaultGameServer()
	fixture.ObjectMeta.Name = "test-pod3"
	fixture.Spec.Ports[0].Protocol = corev1.ProtocolUDP
	fixture.ApplyDefaults()
	// Call the Pod function
	pod, err = fixture.Pod(fakeAPIHooks{})
	// Check for errors
	require.NoError(t, err)
	assert.Equal(t, "test-pod3", pod.Spec.Hostname)
	// Assert that the resulting pod has the expected ContainerPort entries
	assert.Equal(t, 1, len(pod.Spec.Containers[0].Ports))
	// You can further assert the specific values if needed
	assert.Equal(t, int32(7777), pod.Spec.Containers[0].Ports[0].ContainerPort)
	...
	// Test case of static for default(UDP) Protocol
	fixture = defaultGameServer()
	fixture.ObjectMeta.Name = "test-pod4"
	fixture.ApplyDefaults()
	// Call the Pod function
	pod, err = fixture.Pod(fakeAPIHooks{})
	// Check for errors
	require.NoError(t, err)
	assert.Equal(t, "test-pod4", pod.Spec.Hostname)
	// Assert that the resulting pod has the expected ContainerPort entries
	assert.Equal(t, 1, len(pod.Spec.Containers[0].Ports))
	// You can further assert the specific values if needed
	assert.Equal(t, int32(7777), pod.Spec.Containers[0].Ports[0].ContainerPort)
